...
AES256_CTR_DRBG_struct  OQS_NAMESPACE_DRBG_ctx;
...
OQS_NAMESPACE_AES256_ECB(unsigned char *key, unsigned char *ptx, unsigned char *ctx)
{
   uint32_t round_key[4*(NROUNDS + 1)] = {0x00};
    OQS_NAMESPACE_rijndaelKeySetupEnc(round_key, key);
    OQS_NAMESPACE_rijndaelEncrypt(round_key, NROUNDS, ptx, ctx);
}

void
OQS_NAMESPACE_randombytes_init(unsigned char *entropy_input, unsigned char *personalization_string)
{
   ...
   memset(OQS_NAMESPACE_DRBG_ctx.Key, 0x00, 32);
   memset(OQS_NAMESPACE_DRBG_ctx.Vee, 0x00, 16);
    OQS_NAMESPACE_AES256_CTR_DRBG_Update(seed_material, OQS_NAMESPACE_DRBG_ctx.Key,
                                                        OQS_NAMESPACE_DRBG_ctx.Vee);
    OQS_NAMESPACE_DRBG_ctx.reseed_counter = 1;
}
 ...
 */
uint32_t OQS_NAMESPACE_rand_range(const int n, const int logn, AES_XOF_struct *seed_expander_ctx)
{
